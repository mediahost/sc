...
class FacebookControl extends Control
{
	...
	/** @var \App\Model\Facade\ExtUserFacade */
	public $usersModel;

	/** @var \App\Model\Storage\RegistrationStorage */
	public $registration;

	public function __construct(Facebook $fb, \App\Model\Facade\ExtUserFacade $users, \App\Model\Storage\RegistrationStorage $reg)
	{
		parent::__construct();
		$this->facebook = $fb;
        ...
        $dialog->onResponse[] = function (LoginDialog $dialog) {
            ...
            try {
                ...
                if (!$existing = $this->usersModel->findByFacebookId($fb->getUser())) {
					...
					$this->registration->storeFromFacebook($fb->getUser(), $me, $fb->getAccessToken());

					if (($user = $this->usersModel->findByEmail($me->email))) {
						// Merge
//						$this->presenter->redirect('Sign:Merge');
						
						$this->usersModel->mergeFromFacebook($fb->getUser(), $me, $user, $fb->getAccessToken());
